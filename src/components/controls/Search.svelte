...
let origSearchBarLeft = 0;
let origSearchBarWidth = 0;
...
const handleFocus = () => {
	...
	if (searchBarEl && !searchBarEl.style.position) {
		const grid = searchBarEl.closest('#navbar-grid');
		if (grid) {
			const gridRect = grid.getBoundingClientRect();
			const barRect = searchBarEl.getBoundingClientRect();
			origSearchBarLeft = barRect.left - gridRect.left;
			origSearchBarWidth = barRect.width;

			searchBarEl.style.position = 'absolute';
			searchBarEl.style.left = origSearchBarLeft + 'px';
			searchBarEl.style.width = origSearchBarWidth + 'px';

			requestAnimationFrame(() => {
				requestAnimationFrame(() => {
					searchBarEl.style.left = '33.333%';
					searchBarEl.style.width = '33.333%';
				});
			});
		}
	}
...
const handleBlur = () => {
	...
	if (searchBarEl && searchBarEl.style.position === 'absolute') {
		searchBarEl.style.left = origSearchBarLeft + 'px';
		searchBarEl.style.width = origSearchBarWidth + 'px';

		setTimeout(() => {
			if (!searchBarFocused) {
				searchBarEl.style.position = '';
...
<div id="search-bar" bind:this={searchBarEl} class="hidden lg:flex transition-all items-center h-11 rounded-lg
